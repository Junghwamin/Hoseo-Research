...
#
# 리터럴로 쓰지 않고 런타임에 조립한다. INF-04 의 저장소 위생 검사가 추적
# 파일 전체에서 `sk-` + 20자 이상 패턴을 찾는데, 여기에 예외를 두면
# **테스트 파일에 붙여넣은 실제 키를 놓치게 된다**. 그 구멍을 만들지 않으려고
# 소스에 매칭되는 문자열 자체를 남기지 않는다.
# ---------------------------------------------------------------------------
_SK = "sk-"
FAKE_KEY_A = _SK + "test-0000" + "a" * 22   # 34자 (>20, >15)
FAKE_KEY_B = _SK + "test-0000" + "b" * 22
